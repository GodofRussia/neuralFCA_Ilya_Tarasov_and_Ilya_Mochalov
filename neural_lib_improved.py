"""
Improved Neural FCA library with weighted loss for imbalanced datasets
Based on neural_lib58.py with fixes for class imbalance
"""
...
class ConceptNetwork:
    ...
    @classmethod
    def from_lattice(cls, lattice: ConceptLattice, best_concepts_indices: List[int], targets: Tuple[str]):
    ...
    def fit(self, X_df: pd.DataFrame, y: pd.Series, n_epochs: int = 2000,
            loss_fn=None, nonlinearity=torch.nn.Sigmoid):

        X = torch.tensor(X_df[list(self.attributes)].values).float()
        y_tensor = torch.tensor(y.values).long()

        X_train_shuffle, y_train_shuffle = shuffle(X, y_tensor, random_state=random.randint(0, 1000))
        ...
        optimizer = torch.optim.Adam(self.network.parameters(), lr=0.01, betas=(0.9, 0.999))
    ...
    def fit_2(self, X_df: pd.DataFrame, y: pd.Series, n_epochs: int = 2000,
              loss_fn=None, nonlinearity=torch.nn.Sigmoid):
    ...
    def predict_proba(self, X_df: pd.DataFrame) -> torch.Tensor:
        X = torch.tensor(X_df[list(self.attributes)].values).float()
        return self.network(X)

    def predict(self, X_df: pd.DataFrame) -> torch.Tensor:
    ...
    @staticmethod
    def _poset_from_best_concepts(best_concepts: List[FormalConcept], targets: Tuple[str], attrs_tpl: Tuple[str]) -> POSet:
